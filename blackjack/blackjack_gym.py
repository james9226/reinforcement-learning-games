...
class BlackJackEnv(gym.Env):
    ...
    def render(self):
        logger.debug(f'Player Hand : {self.player} with score {sum_hand(self.player)} \n')
        logger.debug(f'Dealer Hand : {self.dealer} with score {sum_hand(self.dealer)} \n')
            
    def get_obs(self) -> dict:
        self.render()
        return {'Dealer': self.dealer[0], 'PlayerCount':sum_hand(self.player), 'PlayerAce': self.has_aces()}
